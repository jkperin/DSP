...
\begin{tikzpicture}[->, >=stealth, shorten >= 0pt, draw=black!50, node distance=\layersep, font=\sffamily]
	...
	\node[block, text width=2cm, align=center] (O1) at (2*\layersep, \dy cm) {Probability of being `A'};
	\node[block, text width=2cm, align=center] (O2) at (2*\layersep, 0 cm) {Probability of being `B'};
	\node[block, text width=2cm, align=center] (O3) at (2*\layersep, -\dy cm) {Probability of being `C'};
    ...
    \node[left = 0mm of xc] (spectrogram) {\resizebox{!}{0.7\textheight}{\includegraphics{figs/speech_spectogram_strip.eps}}};
