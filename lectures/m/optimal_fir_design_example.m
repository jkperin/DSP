...
L = 101;                    % Filter length (number of coefficients)
M = L - 1;                 % Filter order 
w = linspace(0, pi, N).';  % frequency vector omega (N x 1)

% Desired frequency response
d = zeros(size(w)); 
d(w >= 0.2*pi & w <= 0.3*pi) = 10; % bandpass filter between 0.5*pi and 0.7*pi
d(w >= 0.5*pi & w <= 0.7*pi) = 1; % bandpass filter between 0.5*pi and 0.7*pi

% Weight function. 
% Transition band
Deltaw = 0.05*pi; % transition band width
wv  = ones(size(w));
wv(w > 0.2*pi-Deltaw & w < 0.2*pi) = 0; % transition band (don't care)
wv(w > 0.3*pi & w < 0.3*pi+Deltaw) = 0; % transition band (don't care)
